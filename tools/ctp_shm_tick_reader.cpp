...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "usage: ctp_shm_tick_reader <shm_name>" << std::endl;
        return 1;
    }

    try {
        hft_common::ipc::ShmRingBuffer<md_gateway::CtpShmTickRecord> reader(argv[1], false);
        uint64_t next_seq = 1;
        ...
        std::cerr << "ctp_shm_tick_reader failed: " << ex.what() << std::endl;
        return 1;
    }
}
